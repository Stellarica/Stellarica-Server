...
class Components: ChunkComponentInitializer {
	companion object {
		val MULTIBLOCKS = ComponentRegistry.getOrCreate(identifier("multiblocks"), ChunkMultiblocksComponent::class.java);
	}

	override fun registerChunkComponentFactories(registry: ChunkComponentFactoryRegistry) {
		registry.register(MULTIBLOCKS) { ChunkMultiblocksComponent() }
	}
}